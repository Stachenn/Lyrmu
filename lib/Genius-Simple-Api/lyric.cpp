...
void getLyric(std::string songName, std::string availableNameOfCacheFile, std::string varibleToStore[]){
    ...
    lyricSearchSite = std::string("https://lrclib.net/api/search?q=Malcziki");// + formatedSongName);
    ...
    system(std::string("curl -s ""\"" + lyricSearchSite + "\"" + ">> " + availableNameOfCacheFile + ".txt").c_str());
    ...
    cursor = siteContent.find("syncedLyrics\":null");
